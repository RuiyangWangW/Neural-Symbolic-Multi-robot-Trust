#!/usr/bin/env python3
"""
Unified Benchmark System for Multi-Robot Trust Methods

This benchmark combines all test scenarios into a single configurable system:
...
def save_results(
    ...
    if adversarial_lie:
        base_filename = f"adversarial_lie_{config.name}"
    else:
        base_filename = f"{config.name}"
...
def print_benchmark_summary(results: List[Dict], config: BenchmarkConfig):
    ...
    print(f"\n{'=' * 80}")
    print(f"{config.name.upper()} BENCHMARK SUMMARY")
    print(f"{'=' * 80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified benchmark for multi-robot trust methods"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Trust threshold for binary classification (default: 0.3 for supervised objects)"
    )
    ...
    parser.add_argument(
        "--adversarial-lie",
        action="store_true",
        help="Use adversarial track lies evaluation (deceptive mode)"
    )
    ...
    print(f"\n{'=' * 80}")
    print("UNIFIED MULTI-ROBOT TRUST BENCHMARK")
    print(f"{'=' * 80}")
    ...
    for benchmark_name in benchmarks_to_run:
        ...
        results = run_benchmark(
            ...
            base_seed=base_seed,
            adversarial_lie=args.adversarial_lie
        )

        # Save results
        save_results(results, args.output_dir, config, args.adversarial_lie, base_seed)
