...
def save_results(
    ...
    if adversarial_lie:
        base_filename = f"optimized_adversarial_lie_{config.name}"
    else:
        base_filename = f"optimized_{config.name}"
...
def print_benchmark_summary(results: List[Dict], config: BenchmarkConfig):
    ...
    print(f"\n{'=' * 80}")
    print(f"{config.name.upper()} BENCHMARK SUMMARY (Optimized Policy)")
    print(f"{'=' * 80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimized policy benchmark for multi-robot trust methods"
    )
    ...
    parser.add_argument(
        "--adversarial-lie",
        action="store_true",
        help="Use adversarial track lies evaluation (deceptive mode metrics)"
    )
    ...
    print(f"\n{'=' * 80}")
    print("OPTIMIZED POLICY MULTI-ROBOT TRUST BENCHMARK")
    print(f"{'=' * 80}")
    ...
    # Final summary
    print(f"\n{'=' * 80}")
    print("ALL BENCHMARKS COMPLETED (Optimized Policy)")
    print(f"{'=' * 80}")
